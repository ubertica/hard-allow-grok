/**
 * GROK Live Mapper - Async reasoning-to-node extraction
 * Captures Grok's reasoning stream and extracts semantic nodes in real-time
 * Part of Universal Live Context Mapping System for multi-LLM matrix
 */

import EventEmitter from 'events';
import { performance } from 'perf_hooks';

export class MessageStreamAnalyzer extends EventEmitter {
  constructor() {
    super();
    this.buffer = '';
    this.tokens = [];
    this.startTime = null;
  }

  /**
   * Feed reasoning tokens to analyzer (called during streaming)
   * Non-blocking, accumulates tokens in buffer
   */
  feedToken(token) {
    if (!this.startTime) this.startTime = performance.now();

    this.buffer += token;
    this.tokens.push({
      text: token,
      timestamp: performance.now(),
      position: this.tokens.length
    });

    // Emit when buffer reaches meaningful size (paragraph or sentence boundary)
    if (this.buffer.length > 200 || token.includes('\n\n')) {
      this.emit('chunk', this.buffer);
      this.buffer = '';
    }
  }

  /**
   * Signal end of stream, flush remaining buffer
   */
  finalize() {
    if (this.buffer) {
      this.emit('chunk', this.buffer);
    }
    this.emit('end', {
      totalTokens: this.tokens.length,
      duration: performance.now() - this.startTime,
      tokenList: this.tokens
    });
  }

  getBuffer() {
    return this.buffer;
  }

  reset() {
    this.buffer = '';
    this.tokens = [];
    this.startTime = null;
  }
}

export class NodeExtractor {
  constructor() {
    this.nodes = [];
    this.entityPattern = /\b(?:says|claims|decides|determines|realizes|discovers|observes|concludes|infers|deduces|hypothesizes|assumes)\s+(?:that\s+)?(.+?)(?:\.|,|\n)/gi;
    this.relationshipPattern = /(\w+)\s+(?:is|was|are|were|links to|relates to|depends on|affects|causes)\s+(.+?)(?:\.|,|\n)/gi;
  }

  /**
   * Extract entities, relationships, and decisions from text chunk
   */
  extract(text) {
    const nodes = [];
    const entities = this.extractEntities(text);
    const relationships = this.extractRelationships(text);
    const decisions = this.extractDecisions(text);

    // Create nodes from entities
    entities.forEach((entity, idx) => {
      nodes.push({
        id: `entity_${Date.now()}_${idx}`,
        type: 'entity',
        content: entity,
        confidence: 0.85,
        source: 'grok',
        timestamp: Date.now(),
        reasoning: true
      });
    });

    // Create nodes from relationships
    relationships.forEach((rel, idx) => {
      nodes.push({
        id: `relationship_${Date.now()}_${idx}`,
        type: 'relationship',
        content: rel,
        confidence: 0.80,
        source: 'grok',
        timestamp: Date.now(),
        reasoning: true
      });
    });

    // Create nodes from decisions
    decisions.forEach((decision, idx) => {
      nodes.push({
        id: `decision_${Date.now()}_${idx}`,
        type: 'decision',
        content: decision,
        confidence: 0.90,
        source: 'grok',
        timestamp: Date.now(),
        reasoning: true
      });
    });

    this.nodes = [...this.nodes, ...nodes];
    return nodes;
  }

  extractEntities(text) {
    const entities = [];
    const sentences = text.split(/[.!?]/);

    sentences.forEach(sent => {
      const match = sent.match(this.entityPattern);
      if (match) {
        entities.push(sent.trim());
      }
    });

    return entities.slice(0, 5); // Limit to top 5 per chunk
  }

  extractRelationships(text) {
    const relationships = [];
    const matches = text.matchAll(/(\w+)\s+(?:is|links to|depends on|affects)\s+(.+?)(?:\.|,|\n)/gi);

    for (const match of matches) {
      relationships.push({
        source: match[1],
        target: match[2].trim(),
        relation: 'relates_to'
      });
    }

    return relationships.slice(0, 5);
  }

  extractDecisions(text) {
    const decisions = [];
    const decisionKeywords = ['decide', 'conclude', 'determine', 'choose', 'select', 'adopt'];

    decisionKeywords.forEach(keyword => {
      const regex = new RegExp(`(?:i |we |grok )?${keyword}[sd]?\\s+(?:to\\s+)?([^.!?]+)`, 'gi');
      const matches = text.matchAll(regex);
      for (const match of matches) {
        decisions.push({
          action: keyword,
          target: match[1].trim()
        });
      }
    });

    return decisions.slice(0, 3);
  }

  getExtractedNodes() {
    return this.nodes;
  }

  reset() {
    this.nodes = [];
  }
}

export class RelationshipBuilder {
  constructor() {
    this.edges = [];
  }

  /**
   * Create semantic edges between extracted nodes
   */
  buildRelationships(nodes) {
    const edges = [];

    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        const edge = this.createEdge(nodes[i], nodes[j]);
        if (edge) edges.push(edge);
      }
    }

    // Add temporal edges
    nodes.forEach((node, idx) => {
      if (idx < nodes.length - 1) {
        edges.push({
          id: `edge_temporal_${node.id}_${nodes[idx + 1].id}`,
          source: node.id,
          target: nodes[idx + 1].id,
          relation: 'precedes',
          weight: 0.6,
          timestamp: Date.now()
        });
      }
    });

    this.edges = [...this.edges, ...edges];
    return edges;
  }

  /**
   * Smart edge creation based on semantic similarity
   */
  createEdge(nodeA, nodeB) {
    const similarity = this.calculateSimilarity(nodeA.content, nodeB.content);

    if (similarity > 0.5) {
      return {
        id: `edge_${nodeA.id}_${nodeB.id}`,
        source: nodeA.id,
        target: nodeB.id,
        relation: nodeA.type === 'decision' && nodeB.type === 'entity' ? 'applies_to' : 'relates_to',
        weight: similarity,
        timestamp: Date.now()
      };
    }

    return null;
  }

  /**
   * Simple word-overlap similarity
   */
  calculateSimilarity(textA, textB) {
    const wordsA = new Set(textA.toLowerCase().split(/\s+/));
    const wordsB = new Set(textB.toLowerCase().split(/\s+/));

    const intersection = [...wordsA].filter(w => wordsB.has(w)).length;
    const union = new Set([...wordsA, ...wordsB]).size;

    return union > 0 ? intersection / union : 0;
  }

  getEdges() {
    return this.edges;
  }

  reset() {
    this.edges = [];
  }
}

export class ConflictDetector {
  constructor() {
    this.contradictions = [];
    this.learnings = [];
  }

  /**
   * Detect contradictions within extracted nodes
   */
  detectConflicts(nodes) {
    const conflicts = [];

    nodes.forEach((node, idx) => {
      const negationPattern = /(?:not|no|don't|doesn't|won't|can't|never|false|incorrect|wrong)/i;
      const isNegation = negationPattern.test(node.content);

      nodes.slice(idx + 1).forEach(otherNode => {
        if (this.isContradiction(node, otherNode, isNegation)) {
          conflicts.push({
            id: `conflict_${node.id}_${otherNode.id}`,
            nodeA: node.id,
            nodeB: otherNode.id,
            type: 'contradiction',
            severity: 'high',
            timestamp: Date.now(),
            details: `"${node.content}" contradicts "${otherNode.content}"`
          });
        }
      });
    });

    this.contradictions = [...this.contradictions, ...conflicts];
    return conflicts;
  }

  /**
   * Simple contradiction detection via negation/overlap
   */
  isContradiction(nodeA, nodeB, isNegation) {
    if (nodeA.type !== nodeB.type) return false;

    const contentA = nodeA.content.toLowerCase();
    const contentB = nodeB.content.toLowerCase();

    const overlap = contentA.split(/\s+/).filter(w => contentB.includes(w)).length;
    const negationPattern = /(?:not|no|don't|false|wrong)/i;

    return overlap > 3 && (isNegation || negationPattern.test(nodeB.content));
  }

  /**
   * Extract learnings from reasoning process
   */
  extractLearnings(nodes) {
    const learnings = [];
    const learningPatterns = [
      /learned that (.+)/i,
      /realize(?:d)? that (.+)/i,
      /discover(?:ed)? that (.+)/i,
      /understand now that (.+)/i
    ];

    nodes.forEach(node => {
      learningPatterns.forEach(pattern => {
        const match = node.content.match(pattern);
        if (match) {
          learnings.push({
            id: `learning_${node.id}`,
            content: match[1],
            source: node.id,
            timestamp: Date.now(),
            type: 'insight'
          });
        }
      });
    });

    this.learnings = [...this.learnings, ...learnings];
    return learnings;
  }

  getConflicts() {
    return this.contradictions;
  }

  getLearnings() {
    return this.learnings;
  }

  reset() {
    this.contradictions = [];
    this.learnings = [];
  }
}

export class BatchPublisher {
  constructor(queuePath) {
    this.queuePath = queuePath;
    this.batch = [];
    this.batchSize = 10;
    this.publishInterval = 500; // ms
    this.lastPublish = Date.now();
  }

  /**
   * Queue nodes for publication to MatrixHub
   * Non-blocking, buffers and publishes in batch
   */
  queueNodes(nodes, metadata = {}) {
    this.batch.push(
      ...nodes.map(node => ({
        ...node,
        metadata,
        queuedAt: Date.now(),
        llm: 'grok'
      }))
    );

    // Publish if batch full
    if (this.batch.length >= this.batchSize) {
      this.publishBatch();
    } else if (Date.now() - this.lastPublish > this.publishInterval) {
      // Or if timeout reached
      this.publishBatch();
    }
  }

  /**
   * Publish batch asynchronously (fire-and-forget)
   */
  async publishBatch() {
    if (this.batch.length === 0) return;

    const toPublish = [...this.batch];
    this.batch = [];
    this.lastPublish = Date.now();

    // Simulate async publish (non-blocking)
    setImmediate(async () => {
      try {
        // In production: POST to MatrixHub at ~/.grok/hard-allow/matrix-hub.mjs
        // For now: write to queue file
        const fs = await import('fs').then(m => m.promises);
        const path = await import('path').then(m => m.default);

        const queueDir = path.dirname(this.queuePath);
        await fs.mkdir(queueDir, { recursive: true });

        // Append to JSONL queue
        const lines = toPublish.map(item => JSON.stringify(item)).join('\n');
        await fs.appendFile(this.queuePath, lines + '\n');
      } catch (error) {
        console.error('BatchPublisher error:', error.message);
      }
    });
  }

  /**
   * Force publish remaining items
   */
  async flush() {
    if (this.batch.length > 0) {
      await this.publishBatch();
    }
  }
}

export class GrokLiveMapper {
  constructor(queuePath = `${process.env.HOME}/.grok/hard-allow/node-queue-grok.jsonl`) {
    this.analyzer = new MessageStreamAnalyzer();
    this.extractor = new NodeExtractor();
    this.relationshipBuilder = new RelationshipBuilder();
    this.conflictDetector = new ConflictDetector();
    this.publisher = new BatchPublisher(queuePath);

    this.setupListeners();
  }

  setupListeners() {
    this.analyzer.on('chunk', (chunk) => {
      const nodes = this.extractor.extract(chunk);
      const edges = this.relationshipBuilder.buildRelationships(nodes);
      const conflicts = this.conflictDetector.detectConflicts(nodes);
      const learnings = this.conflictDetector.extractLearnings(nodes);

      this.publisher.queueNodes(nodes, {
        edges,
        conflicts,
        learnings
      });
    });

    this.analyzer.on('end', async (stats) => {
      console.log(`[GROK Mapper] Extracted ${this.extractor.getExtractedNodes().length} nodes`);
      await this.publisher.flush();
    });
  }

  /**
   * Start mapping a streaming response
   */
  startMapping() {
    this.analyzer.reset();
    this.extractor.reset();
    this.relationshipBuilder.reset();
    this.conflictDetector.reset();
  }

  /**
   * Feed tokens from Grok response stream
   */
  feedToken(token) {
    this.analyzer.feedToken(token);
  }

  /**
   * End current mapping session
   */
  endMapping() {
    this.analyzer.finalize();
  }

  /**
   * Get current state
   */
  getState() {
    return {
      nodes: this.extractor.getExtractedNodes(),
      edges: this.relationshipBuilder.getEdges(),
      conflicts: this.conflictDetector.getConflicts(),
      learnings: this.conflictDetector.getLearnings()
    };
  }
}

export default GrokLiveMapper;
