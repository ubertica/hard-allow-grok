/**
 * KIMI Live Mapper - Async reasoning-to-node extraction
 * Captures Kimi's reasoning stream and extracts semantic nodes in real-time
...
export class NodeExtractor {
  ...
  extractRelationships(text) {
    const relationships = [];
    const relPatterns = [
      /(\S+)\s+(?:是|属于|关联|影响|导致)\s+(\S+)/g,
      /(\S+)\s+(?:与|和)\s+(\S+)\s+(?:的关系|之间)/g
    ];
  ...
  extractDecisions(text) {
    const decisions = [];
    const decisionKeywords = ['应该', '必须', '可以', '需要', '建议', '提议'];
    ...
    decisionKeywords.forEach(keyword => {
      ...
      for (const match of matches) {
        decisions.push({
          action: keyword,
          target: match[0].trim().slice(0, 100)
        });
      }
    });
...
export class RelationshipBuilder {
  ...
  calculateSimilarity(textA, textB) {
    const wordsA = new Set(textA.split(/[\s\-_]/));
    const wordsB = new Set(textB.split(/[\s\-_]/));
...
export class ConflictDetector {
  ...
  detectConflicts(nodes) {
    ...
    nodes.forEach((node, idx) => {
      const negationPattern = /(?:不|没有|无法|不能|错误|不对)/;
  ...
  isContradiction(nodeA, nodeB, isNegation) {
    ...
    const overlap = contentA.split(/[\s\-_]/).filter(w => contentB.includes(w)).length;
    const negationPattern = /(?:不|没有|无法|不能|错误)/;
    return overlap > 2 && (isNegation || negationPattern.test(nodeB.content));
  }
...
export class KimiLiveMapper {
  constructor(queuePath = `${process.env.HOME}/.grok/hard-allow/node-queue-kimi.jsonl`) {
  ...
  setupListeners() {
    ...
    this.analyzer.on('end', async (stats) => {
      console.log(`[KIMI Mapper] Extracted ${this.extractor.getExtractedNodes().length} nodes`);
